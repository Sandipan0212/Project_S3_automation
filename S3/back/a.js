...
//  mp4Path = "'" + mp4Path + "'";
  ...
  str = str.replaceAll(",", "-");
  const COMPRESS_PATH = "https://s3convertedcdn.ifasonline.com/MP4_Compressed_Video/" + encodeURIComponent(bucket_path_to_upload) + "/" + encodeURIComponent(str) + ".mp4";
  const ENCRYPT_PATH = "https://s3convertedcdn.ifasonline.com/M3U8_VIDEO/" + encodeURIComponent(bucket_path_to_upload) + "/" + encodeURIComponent(str) + "/hls-master.m3u8";
  let compressCommand = "";
  if (data.PATH) {
    compressCommand = "ffmpeg -i '/home/ubuntu/bucket/" + mp4Path + "/" + dir + "' -vcodec libx264 -crf 27  " + compressPath + "/'" + str + ".mp4'";
  } else {
    compressCommand = "ffmpeg -i '/home/ubuntu/bucket/" + dir + "' -vcodec libx264 -crf 27  " + compressPath + "/'" + str + ".mp4";
  }
  const encryptionCommand = `/home/ubuntu/autoscriptcode/packager-linux-x64 'in=${compressPath}/${str}.mp4,stream=audio,init_segment=${encryptPath}/${str}/audio/init.mp4,segment_template=${encryptPath}/${str}/audio/audio$Number$.m4s' 'in=${compressPath}/${str}.mp4,stream=video,init_segment=${encryptPath}/${str}/video/init.mp4,segment_template=${encryptPath}/${str}/video/video$Number$.m4s' --generate_static_live_mpd --mpd_output '${encryptPath}/${str}/mpd-master.mpd' --hls_master_playlist_output '${encryptPath}/${str}/hls-master.m3u8'`;
