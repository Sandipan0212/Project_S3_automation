...
mongoose.connect(MONGODB_URI, { useNewUrlParser: true, useUnifiedTopology: true }).then(async () => {
  ...
  if (serverStatus?.STATUS === 'IDLE') {
    const videoConvertion = await VideoConvertion.find({ COMPRESS_STATUS: 0, ENCRYPT_STATUS: 0,PATH:"test" }).sort({_id:1});
...
//  mp4Path = "'" + mp4Path + "'";
  ...
  let str = data.FILE_NAME.split(".mp4")[0];
  ...
  let compressCommand = "";
  const RESOLUTION=data.RESOLUTION=="360"?'640x360':'1280*720'

if (data.PATH) {
    compressCommand = "ffmpeg -i '/home/ubuntu/bucket/" + mp4Path + "/" + dir + "' -vcodec libx264 -crf 27 -s "+RESOLUTION +" "+ compressPath + "/'" + str + ".mp4'";
  } else {
    compressCommand = "ffmpeg -i '/home/ubuntu/bucket/" + dir + "' -vcodec libx264 -crf 27 -s "+RESOLUTION +" " + compressPath + "/'" + str + ".mp4";
  }

  const encryptionCommand = `/home/ubuntu/autoscriptcode/packager-linux-x64 'in=${compressPath}/${str}.mp4,stream=audio,init_segment=${encryptPath}/${str}/${data.RESOLUTION}/audio/init.mp4,segment_template=${encryptPath}/${str}/${data.RESOLUTION}/audio/audio$Number$.m4s' 'in=${compressPath}/${str}.mp4,stream=video,init_segment=${encryptPath}/${str}/${data.RESOLUTION}/video/init.mp4,segment_template=${encryptPath}/${str}/${data.RESOLUTION}/video/video$Number$.m4s' --generate_static_live_mpd --mpd_output '${encryptPath}/${str}/${data.RESOLUTION}/mpd-master.mpd' --hls_master_playlist_output '${encryptPath}/${str}/${data.RESOLUTION}/hls-master.m3u8'`;
...
console.log("+++++++",currentDirPath)
 if (url === undefined) {
          if (data.PATH) {
            res = s3BucketName + "/" + foldername + "/" + bucket_path_to_upload  + "/" +str+"/"+ data.RESOLUTION ;
          } else {
            res = s3BucketName + "/" + foldername  + "/" +str+"/"+ data.RESOLUTION ;
          }
        } else {
          if (data.PATH) {
